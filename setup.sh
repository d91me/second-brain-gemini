...
authorize_gemini() {
    ...
    if gemini auth status 2>/dev/null | grep -q "Logged in"; then
        success "Gemini CLI already authorized"
        return
    fi

    warn "Gemini CLI needs authorization"
    echo ""
    echo "Run this command manually:"
    echo -e "  ${CYAN}gemini auth login${NC}"
    echo ""
    echo "This will open a browser for authentication."
    echo "After authorizing, the bot will be able to use Gemini for AI processing."
    echo ""
}
